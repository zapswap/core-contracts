...
import { ZapswapPoolSwapTest } from '../typechain/ZapswapPoolSwapTest'
...
describe('ZapswapPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('ZapswapPoolSwapTest')
            const tester = (await testerFactory.deploy()) as ZapswapPoolSwapTest
          ...
          let tester: ZapswapPoolSwapTest
