...
import { TestZapswapSwapPay } from '../typechain/TestZapswapSwapPay'
...
import { TestZapswapReentrantCallee } from '../typechain/TestZapswapReentrantCallee'
...
describe('ZapswapPool', () => {
  ...
  let factory: ZapswapFactory
  let pool: MockTimeZapswapPool

  let swapTarget: TestZapswapCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeZapswapPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestZapswapReentrantCallee')
      ).deploy()) as TestZapswapReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestZapswapSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestZapswapSwapPay')
      underpay = (await underpayFactory.deploy()) as TestZapswapSwapPay
